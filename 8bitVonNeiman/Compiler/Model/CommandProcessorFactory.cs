	...
	public static class CommandProcessorFactory {
		...
		private static class RamRegisterCommands {
			...
			private static DataResponse GetBitArrays(string[] args, string op, CompilerEnvironment env) {
				...
				{
					int num;
					//если метка, берём адресс метки
					if (args[0][1] >= 'a' && args[0][1] <= 'z' || args[0][1] >= 'A' && args[0][1] <= 'Z')
					{
						num = CompilerSupport.ConvertLabelToFarAddress(args[0].Substring(1), env);
						if (num == -1) //если метки не существует
						{
							throw new CompilationErrorExcepton($"Метка {args[0].Substring(1)} не определена", env.GetCurrentLine());
							//из за специфики команды RD невозможно установить в неё адресс метки, которая была объявлена после команды
							/*//dataResponse.highBitArray[4] = true;
							var memoryForLabel = new CompilerEnvironment.MemoryForLabel
							{
								HighBitArray = dataResponse.highBitArray,
								LowBitArray = dataResponse.lowBitArray,
								Address = env.CurrentAddress
							};
							//System.Windows.Forms.MessageBox.Show(memoryForLabel.HighBitArray.ToString());
							env.SetCommandWithoutLabel(memoryForLabel, args[0].Substring(1));//написать аналогичную*/
							/*int address = CompilerSupport.ConvertLabelToFarAddress(p, env);//из .DW
							if (address != -1)
							{
							...
							else
							{
								...
								{
									HighBitArray = dataResponse.highBitArray, //new ExtendedBitArray()
									LowBitArray = dataResponse.lowBitArray, //new ExtendedBitArray()
									Address = env.CurrentAddress,
									SingleByte = false //true
								}, args[0].Substring(1));
							}*/
						}
					}
					else //иначе работаем с числом
					{
						num = CompilerSupport.ConvertToInt(args[0].Substring(1));
					}
					CompilerSupport.FillBitArray(null, dataResponse.lowBitArray, num, Constants.ShortAddressBitsCount);
					dataResponse.highBitArray[4] = true;
				} 
				else 
				{
					...
					if (address == -1) //метка без #
					{
						throw new CompilationErrorExcepton($"Неизвестный аргумент {args[0]}", env.GetCurrentLine());
					}
					CompilerSupport.FillBitArray(null, dataResponse.lowBitArray, address, Constants.ShortAddressBitsCount);
				}
				return dataResponse;
